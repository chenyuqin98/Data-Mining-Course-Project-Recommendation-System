...
description = 'The origin RMSE on valid data is 0.983970' + '\n' + \
              ...
              '9. Feature augment: use CF result as model input' + '\n'
...
if __name__ == '__main__':
    ...
    print('complete CF prediction !!!!!!!!!!!!!!!!!!!!!')

    X_train, Y_train = generate_feature(train_rdd)
    combine_model_train_X = np.array(list(map(lambda x: [x[2], x[3]] + x[4], CF_train_pred)))
    # combine_model_train_X = np.array(list(map(lambda x: [x[2]], CF_train_pred)))
    X_train = np.concatenate((combine_model_train_X, X_train), axis=1)
    ...
    # combine_model_pred_X = np.array(list(map(lambda x: [x[2]], CF_prediction)))
    # X_pred = np.concatenate((combine_model_pred_X, X_pred), axis=1)
